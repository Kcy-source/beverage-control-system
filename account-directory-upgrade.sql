...
create or replace function public.current_app_role() returns text language sql stable security definer set search_path=public as $$ select coalesce((select role from public.app_accounts where lower(email)=lower(auth.jwt()->>'email') limit 1),'user'); $$;
create or replace function public.is_app_admin() returns boolean language sql stable security definer set search_path=public as $$ select public.current_app_role()='admin'; $$;
...
create policy "authenticated can insert inventory" on public.inventory_items for insert to authenticated with check (true);
create policy "authenticated can update inventory" on public.inventory_items for update to authenticated using (true) with check (true);
create policy "authenticated can delete inventory" on public.inventory_items for delete to authenticated using (true);
...
create policy "authenticated can read logs" on public.inventory_logs for select to authenticated using (true);
create policy "authenticated can insert logs" on public.inventory_logs for insert to authenticated with check (true);

-- 提成：Admin 和 User 都可以查看、录入、删除错误记录。
...
create policy "authenticated can read commissions" on public.commission_logs for select to authenticated using (true);
create policy "authenticated can insert commissions" on public.commission_logs for insert to authenticated with check (true);
create policy "authenticated can delete commissions" on public.commission_logs for delete to authenticated using (true);
