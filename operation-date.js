...
(function(){
  ...
  function install(){
    ...
    render=function(){
      ...
      if(body){[...body.rows].forEach((tr,i)=>{const x=logs[i];if(x&&tr.cells[0])tr.cells[0].textContent=dateOf(x);});}
      const cb=document.getElementById('commissionBody');
      if(cb){[...cb.rows].forEach((tr,i)=>{const x=commissions[i];if(x&&tr.cells[0])tr.cells[0].textContent=dateOf(x);});}
      const todayCount=logs.filter(x=>dateOf(x)===today()).length;
      const stat=document.getElementById('statToday');if(stat)stat.textContent=todayCount;
    };
  }
  wait();
})();