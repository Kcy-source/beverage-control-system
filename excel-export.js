...
(function(){
  ...
  function historicalInventoryRows(allLogs,cutoff){
    ...
    items.forEach(x=>states.set(String(x.id),{fridge:0,warehouse:0,baseline:false,activity:false}));
    [...allLogs].sort(logSort).forEach(r=>{
      ...
      if(action==='CREATE'||action==='EDIT'){
        ...
        if(absolute){s.fridge=absolute.fridge;s.warehouse=absolute.warehouse;s.baseline=true;}
        return;
      }
    ...
    const rows=[];let uncertain=0,inconsistent=0;
    [...items].sort((a,b)=>String(a.name).localeCompare(String(b.name))).forEach(x=>{
      ...
      const f=Number(s.fridge||0),w=Number(s.warehouse||0),bad=f<0||w<0;
      if(bad)inconsistent++;
      rows.push({'库存日期':cutoff,'名称':x.name,'类别':x.category||'','规格':x.spec||'','单位':x.unit||'','冰箱库存':f,'仓库库存':w,'总库存':f+w,'最低库存':Number(x.min_quantity||0),'成本价 SGD':Number(x.cost_price||0),'单位提成 SGD':Number(x.commission_per_unit||0),'数据状态':bad?'历史还原异常，请核对':'历史还原'});
    });
    return {rows,uncertain,inconsistent};
  }
  async function exportExcel(){if(!window.XLSX)return alert('Excel 模块正在加载，请稍后重试。');const mode=document.getElementById('excelExportRange').value,value=mode==='day'?document.getElementById('excelExportDay').value:mode==='month'?document.getElementById('excelExportMonth').value:'';if(mode!=='all'&&!value)return alert(mode==='day'?'请选择日期':'请选择月份');const btn=document.getElementById('confirmExcelExport');btn.disabled=true;btn.textContent='导出中...';try{
    ...
    let inventory,inventorySheet='当前库存',uncertain=0,inconsistent=0;
    if(mode==='all')inventory=currentInventoryRows(cutoff);else{const snapshot=historicalInventoryRows(allLogs,cutoff);inventory=snapshot.rows;uncertain=snapshot.uncertain;inconsistent=snapshot.inconsistent;inventorySheet='库存快照';}
    ...
    const wb=XLSX.utils.book_new();addSheet(wb,inventorySheet,inventory);addSheet(wb,'库存记录',logs);addSheet(wb,'销售提成',commission);const label=mode==='all'?'全部':value;XLSX.writeFile(wb,`饮料库存数据_${label}.xlsx`);document.getElementById('exportExcelDialog').close();if(uncertain>0||inconsistent>0){const messages=[];if(uncertain>0)messages.push(`${uncertain} 个品项缺少历史库存基准`);if(inconsistent>0)messages.push(`${inconsistent} 个品项还原后出现负库存`);alert(`Excel 已导出。\n${messages.join('；')}。请查看“数据状态”栏核对。`);}
