// 点开每个饮料时显示该饮料的库存历史，并可按操作、日期和关键字筛选。
(function(){
  let historyRows=[];
  ...
  function ensureHistoryBox(){
    ...
    box.innerHTML=`
      <div class="section-head">
        <h3 style="margin:0;font-size:17px">入库 / 出库历史</h3>
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:10px;margin:12px 0 14px;padding:12px;border:1px solid #e5e7eb;border-radius:12px;background:#fafbfc">
        <label style="font-size:12px;color:#6b7280;display:flex;flex-direction:column;gap:5px">
          ...
          <select id="historyActionFilter">
            ...
            <option value="IN">只看入库</option>
            <option value="OUT">只看出库</option>
            <option value="TRANSFER">只看移库</option>
            <option value="ADJUST">只看盘点</option>
            <option value="CREATE">只看新增</option>
            <option value="EDIT">只看编辑</option>
          </select>
        </label>
        <label style="font-size:12px;color:#6b7280;display:flex;flex-direction:column;gap:5px">
          开始日期
        ...
        <label style="font-size:12px;color:#6b7280;display:flex;flex-direction:column;gap:5px">
          查找
          <input id="historyKeywordFilter" placeholder="备注或操作账号">
        </label>
        <div style="display:flex;align-items:flex-end">
          <button id="historyResetBtn" class="secondary" style="width:100%">清除筛选</button>
        </div>
      </div>
    ...
    document.getElementById('historyResetBtn').onclick=()=>{
      resetHistoryFilters();
      applyHistoryFilters();
    };
  }
  ...
  function resetHistoryFilters(){
    ...
    const keyword=document.getElementById('historyKeywordFilter');
  ...
  async function loadProductHistory(x){
    ...
    const message=document.getElementById('historyFilterMessage');
    if(!body)return;
  ...
  function applyHistoryFilters(){
    ...
    if(from&&to&&from>to){
      body.innerHTML='<tr><td colspan="5">开始日期不能晚于结束日期</td></tr>';
      if(message)message.textContent='请重新选择日期范围。';
      if(summary)summary.textContent='';
      return;
    }
    ...
    const filtered=historyRows.filter(row=>{
      ...
      if(keyword){
        const text=[actionName(row.action),row.note,row.user_email,date].join(' ').toLowerCase();
        if(!text.includes(keyword))return false;
      }
      return true;
    });

    const inQty=filtered.filter(r=>r.action==='IN').reduce((s,r)=>s+Number(r.quantity||0),0);
    const outQty=filtered.filter(r=>r.action==='OUT').reduce((s,r)=>s+Number(r.quantity||0),0);
    if(summary)summary.textContent=`入库 ${inQty} / 出库 ${outQty}`;
    if(message)message.textContent=`显示 ${filtered.length} 条，共 ${historyRows.length} 条记录`;
    ...
    body.innerHTML=filtered.map(r=>`
      ...
      </tr>`).join('')||'<tr><td colspan="5">没有符合筛选条件的记录</td></tr>';
  }
