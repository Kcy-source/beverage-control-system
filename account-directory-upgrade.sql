...
as $$
  select coalesce((select role from public.app_accounts where lower(email)=lower(auth.jwt()->>'email') limit 1),'user');
$$;
...
create policy "admin can read app accounts"
on public.app_accounts for select to authenticated
using (public.is_app_admin());

create policy "user can read own app account"
on public.app_accounts for select to authenticated
using (lower(email)=lower(auth.jwt()->>'email'));

-- 第一位账号可把自己登记为 admin；之后只有 admin 可以新增账号目录。
create policy "admin can add app accounts"
on public.app_accounts for insert to authenticated
with check (
  public.is_app_admin()
  or (
    not exists(select 1 from public.app_accounts)
    and lower(email)=lower(auth.jwt()->>'email')
    and role='admin'
  )
);

create policy "admin can update app accounts"
on public.app_accounts for update to authenticated
using (public.is_app_admin())
with check (public.is_app_admin());

create policy "admin can delete app accounts"
...
-- 产品类别：所有登录账号可查看，只有 admin 可新增/删除。
...
-- 库存产品：所有登录账号可查看和调整库存；只有 admin 可新增/删除产品。
...
create policy "admin can insert inventory" on public.inventory_items for insert to authenticated with check (public.is_app_admin());
create policy "admin can delete inventory" on public.inventory_items for delete to authenticated using (public.is_app_admin());

-- 提成：所有登录账号可查看和录入；只有 admin 可删除错误记录。
...
create policy "admin can delete commissions" on public.commission_logs for delete to authenticated using (public.is_app_admin());
